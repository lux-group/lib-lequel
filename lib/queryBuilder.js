...
const {similarity, to_tsquery, plainto_tsquery, make_tsvector, ts_rank} = require('./functions/index');
...
function queryBuilder(initial_state) {
  ...
  let finalize = function () {
    ...
    if (this.state.from) {
      sql = sql + ` FROM ${(typeof this.state.schema !== 'undefined' ? this.state.schema + '.' : '')}${this.state.from} `
    } else {
      throw "From must be supplied";
    }
    if (this.state.tsquery) {
      sql = sql + ` ,${this.state.tsquery} `
    }
  ...
  let fullTextSearch = function (fields, word, direction) {
    this.state.tsquery = plainto_tsquery(word);
    this.where(make_tsvector(fields, this.state.schema), 'make_tsvector');

    if(typeof direction !== 'undefined'){
      this.addOrderBy(ts_rank(make_tsvector(fields, this.state.schema), direction), direction);
    }
